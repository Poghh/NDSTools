    ...
    try:
        ...
        for row_idx, row_cells in enumerate(source_cells):
            for col_idx, source_cell in enumerate(row_cells):
                dest_cell = dest_ws.cell(row=start_row + row_idx, column=col_idx + 1)
        ...
        for new_min_row, new_max_row, min_col, max_col in merged_ranges:
            try:
                ...
                print(
                    f"Warning: Could not merge cells {new_min_row}:{min_col} to {new_max_row}:{max_col} - {e}"
                )
            ...
            if data_1_sheet_name in dest_wb.sheetnames:
                ...
                for row in range(1, data_1_max_row + 1):
                    for col in range(1, data_1_max_col + 1):
                        ...
                        dest_cell = dest_ws.cell(
                            row=paste_start_row + row - 1, column=paste_start_col + col - 1
                        )
                ...
                for new_min_row, new_max_row, new_min_col, new_max_col in data_1_merged_ranges:
                    try:
                        ...
                        print(
                            f"Warning: Could not merge data_1 cells {new_min_row}:{new_min_col} to {new_max_row}:{new_max_col} - {e}"
                        )
...
def copy_action_data(test_file_path, doc_file_path, progress_callback=None):
    ...
    try:
        ...
        for idx, source_cell in enumerate(data_to_copy):
            dest_cell = dest_ws.cell(row=paste_start_row + idx, column=dest_header_col)
        ...
        print(
            f"Completed copying {len(data_to_copy)} items to アクションNo. column (no merging applied)"
        )
            ...
            if action_data:
                    ...
                    for col in range(1, dest_ws.max_column + 1):
                        ...
                        if cell.value:
                            ...
                            if cell_text in target_columns and target_columns[cell_text] is None:
                                ...
                                print(
                                    f"Found target column '{cell_text}' at row {row}, column {col}"
                                )
                ...
                for target_name, target_info in target_columns.items():
                    ...
                    for idx, source_cell in enumerate(action_data):
                        dest_cell = dest_ws.cell(row=target_start_row + idx, column=target_col)

                        # Copy cell value
                        dest_cell.value = source_cell.value
                        print(
                            f"Copied '{source_cell.value}' to {target_name} column at row {target_start_row + idx}"
                        )
                    ...
                    print(
                        f"Completed copying {len(action_data)} items to {target_name} column (no merging applied)"
                    )
            else:
            ...
            for col in range(1, source_ws.max_column + 1):
                ...
                if cell.value:
                    ...
                    if cell_text == "処理条件":
                        ...
                        print(
                            f"Condition header found at row {condition_header_row}, column {condition_header_col}"
                        )
                        break
        ...
        if condition_header_cell:
            ...
            if condition_data:
                ...
                if target_condition_col is None:
                    ...
                    dest_ws.cell(
                        row=target_condition_row, column=target_condition_col, value="処理条件①"
                    )
                ...
                for idx, source_cell in enumerate(condition_data):
                    dest_cell = dest_ws.cell(
                        row=target_start_row + idx, column=target_condition_col
                    )

                    # Copy cell value
                    dest_cell.value = source_cell.value
                    print(
                        f"Copied '{source_cell.value}' to 処理条件① column at row {target_start_row + idx}"
                    )
            ...
            for col in range(1, source_ws.max_column + 1):
                ...
                if cell.value:
                    ...
                    if cell_text == "API URL":
                        ...
                        api_header_col = col
                        print(
                            f"API URL header found at row {api_header_row}, column {api_header_col}"
                        )
                        break
            if api_header_cell:
                break
        ...
        if api_header_cell:
            ...
            if api_data:
                ...
                for idx, source_cell in enumerate(api_data):
                    dest_cell = dest_ws.cell(row=target_start_row + idx, column=target_webapi_col)

                    # Copy cell value
                    dest_cell.value = source_cell.value
                    print(
                        f"Copied '{source_cell.value}' to WEBAPI column at row {target_start_row + idx}"
                    )
            ...
            for col in range(1, source_ws.max_column + 1):
                ...
                if cell.value:
                    ...
                    if "処理No." in cell_text:  # Contains "処理No." rather than exact match
                        ...
                        print(
                            f"Column containing '処理No.' found at row {shori_no_header_row}, column {shori_no_header_col}: '{cell_text}'"
                        )
                        break
        ...
        if shori_no_header_cell:
            ...
            if shori_no_data:
                ...
                for idx, source_cell in enumerate(shori_no_data):
                    dest_cell = dest_ws.cell(row=target_start_row + idx, column=target_no_col)

                    # Copy cell value
                    dest_cell.value = source_cell.value
                    print(
                        f"Copied '{source_cell.value}' to No. column at row {target_start_row + idx}"
                    )
                ...
                for idx, source_cell in enumerate(shori_no_data):
                    ...
                    current_value = (
                        str(dest_cell.value).strip() if dest_cell.value is not None else ""
                    )
                    if current_value:
                        ...
                        if last_char.isdigit():
                            dest_cell.value = int(
                                last_char
                            )  # Set as integer to avoid text format warning
                        ...
                        print(
                            f"Formatted No. at row {target_start_row + idx}: '{current_value}' → '{last_char}' ({'number' if last_char.isdigit() else 'text'})"
                        )
                    else:
            ...
            for col in range(1, source_ws.max_column + 1):
                ...
                if cell.value:
                    ...
                    if cell_text == "処理概要":
                        ...
                        print(
                            f"処理概要 header found at row {shori_gaiyo_header_row}, column {shori_gaiyo_header_col}"
                        )
                        break
            ...
            for col in range(1, source_ws.max_column + 1):
                ...
                if cell.value:
                    ...
                    if cell_text == "入力パラメータ":
                        ...
                        print(
                            f"入力パラメータ header found at row {nyuryoku_param_header_row}, column {nyuryoku_param_header_col}"
                        )
                        break
        ...
        if shori_gaiyo_header_cell and nyuryoku_param_header_cell:
            ...
            for row in range(start_row, max_data_row + 1):
                ...
                shori_gaiyo_value = (
                    str(shori_gaiyo_cell.value).strip()
                    if shori_gaiyo_cell.value is not None
                    else ""
                )
                nyuryoku_param_value = (
                    str(nyuryoku_param_cell.value).strip()
                    if nyuryoku_param_cell.value is not None
                    else ""
                )
                ...
                if nyuryoku_param_value and nyuryoku_param_value != "-":
                    print(
                        f"Combined data at row {row}: '{combined_value.replace(chr(10), '\\n')}' (処理概要 + 入力パラメータ)"
                    )
                else:
                    print(
                        f"Single data at row {row}: '{combined_value}' (chỉ 処理概要, 入力パラメータ: '{nyuryoku_param_value or 'rỗng'}')"
                    )

                # Store the combined data with source cell for formatting reference (use the first non-empty cell)
                source_cell_for_format = (
                    shori_gaiyo_cell if shori_gaiyo_cell.value is not None else nyuryoku_param_cell
                )
                combined_data.append(
                    {"value": combined_value, "source_cell": source_cell_for_format, "row": row}
                )
            ...
            if combined_data:
                ...
                for idx, data_item in enumerate(combined_data):
                    dest_cell = dest_ws.cell(row=target_start_row + idx, column=target_soutei_col)

                    # Copy combined value
                    dest_cell.value = data_item["value"]
                    print(
                        f"Copied combined value to 想定結果 column at row {target_start_row + idx}"
                    )
        ...
        if no_col is not None and action_no_col is not None and soutei_col is not None:
            print(
                f"DEBUG: Found No. column at {no_col}, アクションNo. column at {action_no_col}, 項目 column at {koumoku_col}, 操作 column at {sousa_col}, 想定結果 column at {soutei_col}"
            )
            ...
            for row in range(2, dest_ws.max_row + 1):  # Start from row 2 to skip headers
                ...
                if (
                    no_cell.value is not None
                    and action_no_cell.value is not None
                    and soutei_cell.value is not None
                ):
                    ...
                    print(
                        f"DEBUG: Row {row} - No='{no_value}', ActionNo='{action_no_value}', First line='{first_line[:50]}...'"
                    )
                    ...
                    print(
                        f"DEBUG: Row {row} skipped - No='{no_cell.value}', ActionNo='{action_no_cell.value}', 想定結果='{soutei_cell.value}'"
                    )
            ...
            for key, group_rows in groups.items():
                ...
                for row_data in group_rows:
                    print(
                        f"    Row {row_data['row']}: No='{row_data['no_value']}', ActionNo='{row_data['action_no_value']}', First line='{row_data['soutei_first_line']}'"
                    )
            ...
            for key, group_rows in groups.items():
                if len(group_rows) > 1:  # Only process groups with multiple rows
                    ...
                    for row_data in group_rows[1:]:  # Skip first row
                        print(
                            f"DEBUG: Processing row {row_data['row']} (No='{row_data['no_value']}', ActionNo='{row_data['action_no_value']}') for deletion"
                        )
            ...
            if rows_to_delete:
                ...
                for row_num in rows_to_delete:
                    ...
                    action_no_value = (
                        str(action_no_cell.value).strip() if action_no_cell.value else "None"
                    )
                    print(
                        f"DEBUG: Deleting row {row_num} (No='{no_value}', ActionNo='{action_no_value}')"
                    )
            ...
            print(
                f"DEBUG: Required columns not found - No.: {no_col}, ActionNo.: {action_no_col}, 想定結果: {soutei_col}"
            )
        ...
        if action_no_col is not None:
            ...
            for row in range(2, current_max_row + 1):  # Start from row 2 to skip headers
                ...
                if action_value:  # Only process non-empty cells
                    if current_group is None or current_group["value"] != action_value:
                        ...
                        current_group = {"value": action_value, "start_row": row, "end_row": row}
                    else:
            ...
            for group in action_groups:
                if group["end_row"] > group["start_row"]:  # Multiple rows
                    ...
                    print(
                        f"DEBUG: Merging ActionNo '{group['value']}' from row {start_row} to {end_row}"
                    )

                    # Merge the cells in ActionNo column
                    dest_ws.merge_cells(
                        start_row=start_row,
                        start_column=action_no_col,
                        end_row=end_row,
                        end_column=action_no_col,
                    )
        ...
        if koumoku_col is not None:
            ...
            for row in range(2, current_max_row + 1):  # Start from row 2 to skip headers
                ...
                if koumoku_value:  # Only process non-empty cells
                    if current_group is None or current_group["value"] != koumoku_value:
                        ...
                        current_group = {"value": koumoku_value, "start_row": row, "end_row": row}
                    else:
                        # Extend current group
                        current_group["end_row"] = row
            ...
            for group in koumoku_groups:
                if group["end_row"] > group["start_row"]:  # Multiple rows
                    ...
                    end_row = group["end_row"]
                    print(
                        f"DEBUG: Merging 項目 '{group['value']}' from row {start_row} to {end_row}"
                    )

                    # Merge the cells in 項目 column
                    dest_ws.merge_cells(
                        start_row=start_row,
                        start_column=koumoku_col,
                        end_row=end_row,
                        end_column=koumoku_col,
                    )
        ...
        if sousa_col is not None:
            ...
            for row in range(2, current_max_row + 1):  # Start from row 2 to skip headers
                ...
                if sousa_value:  # Only process non-empty cells
                    if current_group is None or current_group["value"] != sousa_value:
                        ...
                        current_group = {"value": sousa_value, "start_row": row, "end_row": row}
                    else:
                        # Extend current group
                        current_group["end_row"] = row
            ...
            for group in sousa_groups:
                if group["end_row"] > group["start_row"]:  # Multiple rows
                    ...
                    end_row = group["end_row"]
                    print(
                        f"DEBUG: Merging 操作 '{group['value']}' from row {start_row} to {end_row}"
                    )

                    # Merge the cells in 操作 column
                    dest_ws.merge_cells(
                        start_row=start_row,
                        start_column=sousa_col,
                        end_row=end_row,
                        end_column=sousa_col,
                    )
        ...
        result = {
            ...
            "shori_no_data_copied": len(shori_no_data)
            if "shori_no_data" in locals() and shori_no_data
            else 0,
            "no_target_created": "No." if shori_no_header_cell else "None",
            "no_data_formatted": len(shori_no_data)
            if "shori_no_data" in locals() and shori_no_data
            else 0,
            "combined_columns_found": shori_gaiyo_header_cell is not None
            and nyuryoku_param_header_cell is not None,
            "combined_data_copied": len(combined_data)
            if "combined_data" in locals() and combined_data
            else 0,
            "soutei_target_created": "想定結果"
            if "shori_gaiyo_header_cell" in locals()
            and shori_gaiyo_header_cell
            and "nyuryoku_param_header_cell" in locals()
            and nyuryoku_param_header_cell
            else "None",
...
def validate_file_compatibility(test_file_path, doc_file_path):
    ...
    try:
        ...
        if (test_ext, doc_ext) in supported_combinations:
            return {"compatible": True, "message": f"File {test_ext} và {doc_ext} tương thích!"}
        else:
            return {
                "compatible": False,
        ...
        return {"compatible": False, "message": f"Lỗi kiểm tra tương thích: {str(e)}"}

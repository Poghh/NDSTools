...
try:
    ...
    import openpyxl
    from openpyxl.worksheet.views import Selection, SheetView
    ...
    for item in os.listdir(base_folder):
        ...
        if os.path.isdir(item_path):
            ...
            for screen in screen_list:
                ...
                if screen_clean and (
                    screen_clean.lower() in item.lower() or item.lower() in screen_clean.lower()
                ):
    ...
    try:
        ...
        if has_objects and not force_format:
            if output_callback:
                ...
                output_callback("   Để bảo vệ linked objects, tool sẽ BỎ QUA file này")
                output_callback("  💡 Bật 'Force Format' nếu muốn xử lý dù có rủi ro")
            ...
            if output_callback:
                ...
                output_callback("  🚨 CẢNH BÁO: Linked objects có thể bị mất!")
...
def process_screen_folders(screen_list, base_folder, output_callback=None, force_format=False):
    ...
    for screen_name, folder_path in found_folders.items():
        ...
        if output_callback:
            output_callback(
                f" Kết quả {screen_name}: {success_count} thành công, {error_count} lỗi\n"
            )
            output_callback("-" * 50 + "\n")

...
from toolsAction.utAction.copy_data import (
    copy_action_data,
    copy_data_action,
    get_action_copy_preview,
    get_copy_preview,
...
class UnitTestTab:
    ...
    def init_ui(self):
        ...
        # Left section header
        ctk.CTkLabel(
            left_frame, text="Tải file tài liệu mocks", font=ctk.CTkFont(size=14, weight="bold")
        ...
        ctk.CTkLabel(
            right_frame, text="Tải file unit test", font=ctk.CTkFont(size=14, weight="bold")
        ).grid(row=0, column=0, columnspan=2, padx=16, pady=(16, 8), sticky="w")
        ...
        self.preview_text = ctk.CTkTextbox(
            self.preview_frame, height=250, font=ctk.CTkFont(size=11)
        )
        ...
        try:
            ...
            if self.current_action == "action_copy":
                result = copy_action_data(
                    self.test_file_path, self.doc_file_path, progress_callback=progress_update
                )
